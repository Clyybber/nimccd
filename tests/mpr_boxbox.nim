...
block mprBoxboxAlignedX:
    ...
    for i in 0..<100:
        res = intersectMPR(addr box1, addr box2, ccd)
        if i < 35 or i > 65:
            assert not res
        elif i != 35 and i != 65:
            assert res
...
block mprBoxboxPenetration:
    ...
    #[
    box2.pos = vec3[real](0, 0, 0)
    res = penetrationMPR(addr box1, addr box2, ccd, depth, dir, pos)
    assert res
    ...
    box1.y = 1
    box1.x = 1
    box2.x = 0.5; box2.y = 0.5; box2.z = 0.5
    box1.pos = vec3[real](0, 0, 0)
    box1.quat = quat[real](0, 0, 0, 1)
    box2.pos = vec3[real](0, 0.73, 0)
    box2.quat = quat[real](0, 0, 0, 1)

    res = penetrationMPR(addr box1, addr box2, ccd, depth, dir, pos)
    assert res
    recPen(depth, dir, pos, "Pen 9")
    #TOSVT()
    ...
    box1.pos = vec3[real](0, 0, 0)
    box1.quat = quat[real](0, 0, 0, 1)
    box2.pos = vec3[real](0.3, 0.738, 0)
    box2.quat = quat[real](0, 0, 0, 1)

    res = penetrationMPR(addr box1, addr box2, ccd, depth, dir, pos)
    assert res
    recPen(depth, dir, pos, "Pen 10")
    #TOSVT()
