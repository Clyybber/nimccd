...
block cylcylAlignedX:
    ...
    for i in 0..<100:
        res = intersectGJK(addr c1, addr c2, ccd)
...
template TOSVT(): untyped =
    svtObjPen(addr cyl1, addr cyl2, "Pen 1", depth, dir, pos)
    dir *= depth
    cyl2.pos += dir
    svtObjPen(addr cyl1, addr cyl2, "Pen 1", depth, dir, pos)

block cylcylPenetrationEPA:
    var ccd = initCCD[ptr CollisionObj[real], real]()
    ...
    res = penetrationGJK(addr cyl1, addr cyl2, ccd, depth, dir, pos)
    assert res
